...
export function parseCart(raw) {
  try {
    ...
    return Array.isArray(items) ? items.filter((item) => item && typeof item === 'object' && typeof item.id === 'string' && typeof item.name === 'string' && Number.isInteger(item.quantity) && item.quantity > 0 && item.quantity <= 500) : [];
  } catch { return []; }
}
