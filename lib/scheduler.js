...
const isWorkSession = (date, settings) => (
  settings.workDays.includes(date.getDay()) && !settings.daysOff.includes(dateKey(date))
);
...
const buildSessions = (settings, count = 180) => {
  const validDays = (settings.workDays || []).filter((day) => Number.isInteger(day) && day >= 0 && day <= 6);
  if (!validDays.length || !(Number(settings.minutesPerSession) > 0)) return [];
  settings = { ...settings, workDays: validDays };
  const sessions = [];
  let cursor = new Date();
...
export const turnaround = (schedule) => {
  ...
  if (!lastOrder) {
    ...
    return {
      ...
      detail: 'Does not include shipping time.',
    };
  }
  const completion = schedule.results[lastOrder.id]?.completionDate;
  return completion
    ? { label: `Ready by ${formatDate(completion)}`, detail: 'Studio finish date — shipping is extra if your order ships.' }
    : { label: 'New orders are being reviewed', detail: 'Please contact us for a completion estimate. Shipping time is extra if your order ships.' };
};
