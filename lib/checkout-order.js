...
import { parseCustomQuantity } from './custom-order.js';
...
export function resolveCheckoutOrder(submittedItems, products, submittedAt = new Date().toISOString()) {
  ...
  for (const [index, submittedItem] of items.entries()) {
    ...
    if (!parseCustomQuantity(item.quantity)) {
      throw Object.assign(new Error('Choose a whole-number quantity between 1 and 500.'), { status: 400 });
    }
    ...
    resolved.push({
      ...
      custom_details: {
        ...((item.isCustom || item.is_custom) ? Object.fromEntries(['type', 'item', 'color', 'personalization', 'details', 'neededBy'].map((key) => [key, String(item.custom_details?.[key] || '').slice(0, 5000)])) : {}),
        ...((item.isCustom || item.is_custom) ? { quantity: String(item.quantity) } : {}),
        ...proof,
      },
    });
